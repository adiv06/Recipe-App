...
struct RecipeView: View {
   ...
   /* func populateIngredients() -> [String]{
        var returnedIngredientNames: [String]
        ForEach(recipe.analyzedInstructions[0]?.steps ?? [Step(number: 1, step: "Could not find any steps...")], id: \.self) { step in

            ForEach(step.ingredients ?? [Ingredient(id: 1, name: "Could not find any ingredients for this particular recipe...", image: "no image")]) { ingredient in
                returnedIngredientNames.append(ingredient.name)
            }
             
        }
        return returnedIngredientNames
    }*/
        
   /* func printIngredients(recipeInstructions: [AnalyzedInstruction?]){
        for instruction in recipeInstructions {
            for step in instruction?.steps ?? [Step(number: 1, step: "LLLLLLLL")]{
